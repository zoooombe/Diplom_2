...
class TestUserCreation:
    ...
    def test_create_duplicate_user(self, create_duplicate_user):
        with allure.step('Отправляем запрос на создание дубликата пользователя'):
    ...
    @allure.title(
        'Тест на попытку создать пользователя и не заполнить одно из обязательных полей, ручка: /api/auth/register')
    ...
    def test_create_user_with_missing_fields(self, registration_data):
        with allure.step('Отправляем запрос на создание пользователя с отсутствующим полем'):
