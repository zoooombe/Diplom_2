...
class TestUserLogin:
    ...
    def test_login_current_user(self, create_duplicate_user):
        with allure.step('Отправляем запрос на вход существующего пользователя'):
    ...
    @allure.title(
        'Тест на попытку входа с неверным логином и паролем, ручка: /api/auth/register, ручка: /api/auth/login')
    def test_login_user_with_wrong_login_and_password(self):
        ...
        with allure.step('Проверяем ответ с ошибкой авторизации'):
